...
export async function resolveWikidataSource({ wikidata_id, enrich } = {}, deps = {}) {
  const getFilm = deps.getWikidataFilm || getWikidataFilm;
  const getWiki = deps.fetchMediawikiPlot || fetchMediawikiPlot;
  const film = await getFilm(wikidata_id);
  let synopsis = text(film.overview);
  let mediawiki = null;
  const wantWiki = text(enrich).toLowerCase() !== "none";
  if (wantWiki) {
    try {
      mediawiki = await getWiki(film.enwiki_title || film.title);
      if (text(mediawiki?.plot)) {
        synopsis = [synopsis, text(mediawiki.plot)].filter(Boolean).join("\n\n");
      }
    } catch {
      // description-only when wiki unavailable
    }
  }
  if (!synopsis) {
    throw new Error(`Wikidata item ${film.id} has no description or MediaWiki plot`);
  }
  return {
...
export async function resolveArchiveSource({ archive_id, enrich } = {}, deps = {}) {
  const getFilm = deps.getArchiveFilm || getArchiveFilm;
  const getWiki = deps.fetchMediawikiPlot || fetchMediawikiPlot;
  const film = await getFilm(archive_id);
  let synopsis = text(film.overview);
  let mediawiki = null;
  const wantWiki = text(enrich).toLowerCase() !== "none";
  if (wantWiki) {
    try {
      mediawiki = await getWiki(film.title);
      if (text(mediawiki?.plot)) {
        synopsis = [synopsis, text(mediawiki.plot)].filter(Boolean).join("\n\n");
      }
    } catch {
  ...
  if (!synopsis) {
    throw new Error(`Archive.org item ${film.id} has no description or MediaWiki plot`);
  }
  return {
    title: film.title || film.id,
    synopsis,
    source: {
      ...
      year: film.year || "",
      poster_url: film.poster_url || "",
      url: film.url || `https://archive.org/details/${film.id}`,
      mediawiki_title: mediawiki?.title || null,
      synopsis
    }
  };
...
export function stableDnaId(sourceKind, externalId) {
  ...
  if (kind === "gutendex") {
    ...
    if (!Number.isInteger(id) || id < 1) {
      throw new Error("stableDnaId gutendex requires positive integer id");
    }
    return `gutendex-${id}`;
  }
  if (kind === "wikidata") {
    const id = normalizeWikidataId(externalId);
    if (!id) throw new Error("stableDnaId wikidata requires Q-id");
    return `wikidata-${id}`;
  }
  if (kind === "archive") {
    const id = normalizeArchiveId(externalId);
    if (!id) throw new Error("stableDnaId archive requires valid identifier");
    return `archive-${id}`;
  }
  throw new Error("stableDnaId requires wikidata|archive|gutendex");
}
  ...
  return readdirSync(dir)
    ...
    .map((name) => {
      ...
      return {
        ...
        external_id:
          record?.source?.wikidata_id ||
          record?.source?.archive_id ||
          record?.source?.gutendex_id ||
          record?.source?.tmdb_id ||
          null,
...
export async function ingestStoryDna(input = {}, deps = {}) {
  ...
  const resolveWikidata = deps.resolveWikidata || resolveWikidataSource;
  const resolveArchive = deps.resolveArchive || resolveArchiveSource;
  ...
  const wikidataId = normalizeWikidataId(input.wikidata_id || input.qid || "");
  const archiveId = normalizeArchiveId(input.archive_id || input.identifier || "");
  const gutendexId = input.gutendex_id != null ? Number(input.gutendex_id) : null;
  const enrich = input.enrich;

  let kind = sourceKind;
  let externalId = null;
  if (kind === "wikidata" || (wikidataId && !kind)) {
    kind = "wikidata";
    externalId = wikidataId || normalizeWikidataId(input.wikidata_id);
  } else if (kind === "archive" || (archiveId && !kind)) {
    kind = "archive";
    externalId = archiveId || normalizeArchiveId(input.archive_id);
  } else if (kind === "gutendex" || (gutendexId && !kind)) {
    kind = "gutendex";
    externalId = gutendexId;
  } else {
    const error = new Error("Ingest requires source wikidata|archive|gutendex and id (manual text ingest disabled)");
    error.status = 400;
    throw error;
  }
  if (kind === "gutendex" && (!Number.isInteger(externalId) || externalId < 1)) {
    const error = new Error("gutendex_id must be a positive integer");
    error.status = 400;
    throw error;
  }
  if (kind === "wikidata" && !normalizeWikidataId(externalId)) {
    const error = new Error("wikidata_id must look like Q123");
    error.status = 400;
    throw error;
  }
  if (kind === "archive" && !normalizeArchiveId(externalId)) {
    const error = new Error("archive_id is invalid");
    error.status = 400;
    throw error;
  }
  ...
  let resolved;
  if (kind === "wikidata") {
    resolved = await resolveWikidata({ wikidata_id: externalId, enrich }, deps);
  } else if (kind === "archive") {
    resolved = await resolveArchive({ archive_id: externalId, enrich }, deps);
  } else {
