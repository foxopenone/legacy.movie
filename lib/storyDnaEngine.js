...
import { fetchMediawikiPlot, getWikidataFilm, normalizeWikidataId } from "./storyDnaCatalog.js";
...
export function stableDnaId(sourceKind, externalId) {
  ...
  throw new Error("stableDnaId requires wikidata|gutendex");
}
  ...
  return readdirSync(dir)
    ...
    .map((name) => {
      ...
      return {
        ...
        external_id: record?.source?.wikidata_id || record?.source?.gutendex_id || record?.source?.tmdb_id || null,
...
export async function ingestStoryDna(input = {}, deps = {}) {
  ...
  if (kind === "wikidata" || (wikidataId && !kind)) {
    ...
    const error = new Error("Ingest requires source wikidata|gutendex and id (manual text ingest disabled)");
    error.status = 400;
    throw error;
  }
