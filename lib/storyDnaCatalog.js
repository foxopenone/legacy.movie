...
export async function searchWikidataFilms(query, { limit = 12 } = {}) {
  ...
  const url =
    ...
    `&language=en&uselang=en&type=item&limit=${cap}&format=json&origin=*`;
  const payload = await fetchJson(url);
  ...
  const results = ids.map((id) => projectWikidataFilm(entities[id])).filter((item) => item.id && item.title);
  return {
    page: 1,
    total_pages: 1,
    total_results: results.length,
...
export async function searchCatalogCross(query, { limit = 8 } = {}) {
  ...
  return {
    ...
    errors,
    attribution: "Wikidata · Project Gutenberg via Gutendex"
  };
}
