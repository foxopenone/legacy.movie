...
function preferWithPoster(items = []) {
  return [...(Array.isArray(items) ? items : [])].sort((a, b) => {
    const ap = text(a?.poster_url) ? 0 : 1;
    const bp = text(b?.poster_url) ? 0 : 1;
    return ap - bp;
  });
}

/** Wikipedia pageimages — many films lack Wikidata P18 but have enwiki posters. */
async function fetchWikipediaThumbnails(titles = []) {
  const list = [...new Set((Array.isArray(titles) ? titles : []).map(text).filter(Boolean))].slice(0, 50);
  if (!list.length) return {};
  const url =
    `${WIKI_API}?action=query&prop=pageimages` +
    `&titles=${list.map(encodeURIComponent).join("|")}` +
    `&pithumbsize=342&pilicense=any&format=json&origin=*`;
  const payload = await fetchJson(url, undefined, 20000);
  ...
  for (const page of pages) {
    ...
    const thumb = text(page?.thumbnail?.source);
    if (title && thumb) map[title] = thumb;
  }
  // Map request titles → normalized titles so sitelink lookups still hit.
  for (const row of Array.isArray(payload?.query?.normalized) ? payload.query.normalized : []) {
    const from = text(row?.from);
    const to = text(row?.to);
    if (from && to && map[to] && !map[from]) map[from] = map[to];
  }
  return map;
}

async function fillMissingWikidataPosters(items = [], entitiesById = null) {
  const list = Array.isArray(items) ? items : [];
  const missing = list.filter((item) => item?.id && !text(item.poster_url));
  if (!missing.length) return preferWithPoster(list);

  let entities = entitiesById && typeof entitiesById === "object" ? entitiesById : null;
  if (!entities) {
    entities = await getWikidataEntities(missing.map((item) => item.id));
  }
  ...
  for (const item of missing) {
    ...
    const wikiTitle =
      text(item.enwiki_title) ||
      text(entity?.sitelinks?.enwiki?.title);
    if (wikiTitle) titleById[item.id] = wikiTitle;
  }
  const thumbs = await fetchWikipediaThumbnails(Object.values(titleById));
  const filled = list.map((item) => {
    if (text(item.poster_url)) return item;
    const title = titleById[item.id];
    const poster = title ? text(thumbs[title]) : "";
    return poster ? { ...item, poster_url: poster } : item;
  });
  return preferWithPoster(filled);
}
...
function projectWikidataFilm(entity, { genreLabels = {} } = {}) {
  ...
  return {
    ...
    source_url: id ? `https://www.wikidata.org/wiki/${id}` : "",
...
/** Curated famous films — wbgetentities is fast; SPARQL popular browse was timing out (50s+ / 504). */
const WIKIDATA_POPULAR_FILMS = [
  "Q25188", "Q83495", "Q163872", "Q190050", "Q172241", "Q184843", "Q181795", "Q181803",
  "Q165325", "Q17738", "Q103747", "Q104123", "Q189540", "Q208217", "Q47245", "Q134773",
  "Q164963", "Q181803", "Q200286", "Q220776", "Q24815", "Q271189", "Q279630", "Q309156",
  "Q319191", "Q327763", "Q42193", "Q44578", "Q482645", "Q483941", "Q485310", "Q496755",
  "Q54454", "Q57132", "Q6074", "Q653500", "Q686656", "Q72962", "Q74388", "Q76193",
  "Q85371", "Q860459", "Q918209", "Q102225", "Q11621", "Q132014", "Q150804", "Q153723",
  "Q161551", "Q170564", "Q181598", "Q192212", "Q200905", "Q210271", "Q221216", "Q230552",
  "Q251363", "Q270730", "Q279627", "Q309003", "Q43303", "Q46535", "Q482682", "Q496719",
  "Q58239", "Q62836", "Q72887", "Q80944", "Q945282", "Q103254", "Q128547", "Q151073",
  "Q161688", "Q170738", "Q182015", "Q200884", "Q214841", "Q222753", "Q238993", "Q270482",
];
...
export function listWikidataCategories() {
  return WIKIDATA_CATEGORIES.map(({ id, label }) => ({ id, label }));
}

export async function listWikidataPopular({ page = 1, category = "all" } = {}) {
  const pageNum = Math.max(1, Math.min(50, Number(page) || 1));
  const pageSize = 24;
  const uniqueIds = [...new Set(WIKIDATA_POPULAR_FILMS.map(normalizeWikidataId).filter(Boolean))];
  const entities = await getWikidataEntities(uniqueIds);
  const cat = WIKIDATA_CATEGORIES.find((item) => item.id === text(category)) || WIKIDATA_CATEGORIES[0];
  let rows = uniqueIds
    .map((id) => entities[id])
  ...
  if (cat.min != null && cat.max != null) {
    rows = rows.filter((item) => {
      ...
      return Number.isFinite(year) && year >= cat.min && year <= cat.max;
    });
  }
  rows = await fillMissingWikidataPosters(rows, entities);
  const total = rows.length;
  const offset = (pageNum - 1) * pageSize;
  const results = rows.slice(offset, offset + pageSize).map((item) => ({
    ...item,
  ...
  return {
    ...
    total_results: total,
    categories: listWikidataCategories(),
    category: cat.id,
    results,
...
`.trim();
  ...
  try {
    ...
    if (results.length) {
      const filled = await fillMissingWikidataPosters(results);
      return {
        page: 1,
        total_pages: 1,
        total_results: filled.length,
        results: filled.slice(0, cap),
        attribution: "Film data from Wikidata"
      };
    }
  } catch {
  ...
  const projected = ids
    ...
    .map((entity) => projectWikidataFilm(entity))
    .filter((item) => item.id && item.title);
  const results = (await fillMissingWikidataPosters(projected, entities)).slice(0, cap);
  return {
    page: 1,
    total_pages: 1,
...
export async function getWikidataFilm(wikidataId) {
  ...
  const [item] = await fillMissingWikidataPosters(
...
export function listGutendexCategories() {
  return GUTENDEX_TOPICS.map(({ id, label }) => ({ id, label }));
}

export async function listGutendexPopular({ page = 1, topic = "all" } = {}) {
  const pageNum = Math.max(1, Math.min(100, Number(page) || 1));
  const topicKey = text(topic);
  const useTopic = topicKey && topicKey !== "all";
  const url = useTopic
    ? `${GUTENDEX_BASE}/books/?topic=${encodeURIComponent(topicKey)}&page=${pageNum}`
    : `${GUTENDEX_BASE}/books/?sort=popular&page=${pageNum}`;
  const payload = await fetchJson(url, undefined, 25000);
  const results = preferWithPoster(
    Array.isArray(payload?.results) ? payload.results.map(projectGutendexBook) : []
  );
  ...
  return {
    ...
    total_results: count,
    categories: listGutendexCategories(),
    category: useTopic ? topicKey : "all",
...
const ARCHIVE_COLLECTIONS = [
  ...
  { id: "classic_movies", label: "Classic movies" },
  { id: "publicdomain", label: "Public domain" },
  { id: "SciFi_Horror", label: "Sci-Fi / Horror" },
  { id: "silent_films", label: "Silent films" },
];

export function listArchiveCategories() {
  return ARCHIVE_COLLECTIONS.map(({ id, label }) => ({ id, label }));
}

export async function listArchivePopular({ page = 1, collection = "feature_films" } = {}) {
  const pageNum = Math.max(1, Math.min(200, Number(page) || 1));
  const rows = 24;
  const allowed = new Set(ARCHIVE_COLLECTIONS.map((item) => item.id));
  const col = allowed.has(text(collection)) ? text(collection) : "feature_films";
  const q = `(collection:${col} AND mediatype:movies)`;
  const url =
  ...
  return {
    ...
    total_results: total,
    categories: listArchiveCategories(),
    category: col,
    results,
